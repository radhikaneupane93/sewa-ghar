    ...
    const fetchClothBanks = async () => {
      try {
        const response = await axios.get(
          "http://127.0.0.1:8000/donation/clothbanks/"
        );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 mb-4 mt-8 text-center">
        These locations provide you with the opportunity to make a positive
        impact through your donations, and also offer recognition for your
        contributions. Select the nearest cloth bank and donate today!
      </p>
      <div className="flex flex-col mt-8">
        {clothBanks.map((bank, index) => (
          <div
            className="bg-white rounded-lg shadow-md p-6 mb-4 flex gap-8 mx-auto w-full"
            onClick={() => navigateToDonation(bank.id, bank.title)}
          >
            <ClothLocation
              key={index}
              title={bank.title}
